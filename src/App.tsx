...
export default function App() {
  ...
  const handleLogin = async (e?: { preventDefault?: () => void }) => {
    ...
    if (u === 'phokyluat' && p === 'bcs1234') { setRole('viceDiscipline'); setLinkedStudentId(null); setLoggedIn(true); showToast('LP kỷ luật'); return }
    {
      ...
      if (child) {
        const phoneTail = (child.parentPhone || '').replace(/\D/g, '').slice(-4)
        if (p === 'view1234' || (phoneTail && p === phoneTail) || (child.parentCode && p === child.parentCode)) {
      ...
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'linear-gradient(135deg,#ecfdf5,#f0fdfa,#fff)' }}>
        <div className="bg-white rounded-3xl shadow-xl border border-emerald-100 p-8 w-full max-w-md">
          ...
          <p className="text-xs text-gray-400 text-center leading-relaxed">
            GVCN: quanlyhocsinh / qlhs1234<br />
            PH từng HS: SĐT (vd 0901234567) / view1234<br />
      ...
      {modal?.type === 'editStudent' && modal.studentId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 print:hidden">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-md space-y-3">
            ...
            <input value={editForm.parentCode} onChange={e => setEditForm(f => ({ ...f, parentCode: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Mã đăng nhập PH" />
            <p className="text-xs text-gray-500">PH đăng nhập: SĐT hoặc mã / view1234 (hoặc 4 số cuối SĐT)</p>
