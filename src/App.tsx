...
export default function App() {
  ...
  const addTransaction = (studentId: string, ruleId: string, date: string) => {
    ...
    if (isFirebaseConfigured) {
      upsertDoc('scoreTransactions', tx.id, tx as unknown as Record<string, unknown>).catch(console.error)
    }
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-6 flex flex-wrap gap-3">
                <button onClick={() => {
                  ...
                  transactions.forEach(t => {
                    ...
                    rows.push([t.id, s?.fullName || t.studentId, String(t.weekNumber), t.date, r?.name || t.ruleId, String(t.points), t.createdBy])
                  })
                  downloadCSV('lich-su-giao-dich.csv', rows)
                  showToast('Đã xuất lịch sử giao dịch CSV')
                }} className="border border-emerald-600 text-emerald-700 px-4 py-2 rounded-xl text-sm font-medium hover:bg-emerald-50">Xuất lịch sử điểm (CSV)</button>
                {!demoLoaded && (
