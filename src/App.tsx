...
import {
  ...
  createScoreTransactionAtomic, setWeekLocked, writeAuditLog,
        ...
        <main className="flex-1 p-4 md:p-6 max-w-full overflow-x-hidden">
          ...
          {page === 'attendance' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div><h2 className="text-2xl font-bold text-emerald-900">Điểm danh nhanh</h2><p className="text-sm text-emerald-600">Ghi chuyên cần / vắng / trễ cho cả lớp</p></div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-x-auto">
                <table className="w-full text-sm min-w-[600px]">
                  <thead className="bg-emerald-50"><tr><th className="px-3 py-2 text-left">STT</th><th className="px-3 py-2 text-left">Họ tên</th><th className="px-3 py-2 text-left">SĐT PH</th><th className="px-3 py-2 text-center">Trạng thái ngày</th></tr></thead>
                  <tbody>
                    {filteredStudents.map(s => {
                      ...
                      return (
                        <tr key={s.id} className="border-t border-gray-50">
                          ...
                          <td className="px-3 py-2 text-center">{r ? `${r.icon} ${r.name}` : <span className="text-gray-400">Chưa điểm danh</span>}</td>
                        </tr>
                      )
                    })}
                  </tbody>
          ...
          {page === 'timetable' && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-emerald-600">Nhập môn / bài học theo tiết · lưu local + Firebase</p>
              <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-x-auto">
                <table className="w-full text-sm min-w-[600px]">
          ...
          {page === 'students' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div><h2 className="text-2xl font-bold text-emerald-900">Học sinh / Liên hệ PH</h2><p className="text-sm text-emerald-600">Import CSV · SĐT phụ huynh</p></div>
                <div className="flex flex-wrap gap-2">
                  {canEditStudents(role) && (<>
                    <button onClick={() => fileImportRef.current?.click()} className="border border-teal-600 text-teal-700 px-3 py-2 rounded-xl text-sm font-medium hover:bg-teal-50 flex items-center gap-1"><Upload className="w-4 h-4" /> Import CSV</button>
                    ...
                    {filteredStudents.map(s => {
                      ...
                      return (
                        <tr key={s.id} className="border-t border-gray-50 hover:bg-emerald-50/50">
                          ...
                          <td className="px-3 py-2.5">{s.parentPhone ? <a href={`tel:${s.parentPhone}`} className="text-emerald-700 text-xs flex items-center gap-1"><Phone className="w-3.5 h-3.5" />{s.parentPhone}</a> : <span className="text-gray-400">—</span>}</td>
                          <td className={`px-3 py-2.5 text-center font-semibold ${sc > 0 ? 'text-green-600' : sc < 0 ? 'text-red-500' : ''}`}>{sc}</td>
                          <td className="px-3 py-2.5 text-center">
                            ...
                            {canEditStudents(role) && <button onClick={() => { if (!confirm(`Xóa ${s.fullName}?`)) return; setStudents(prev => prev.map(x => x.id === s.id ? { ...x, active: false } : x)); if (isFirebaseConfigured) upsertDoc('students', s.id, { active: false }).catch(console.error); showToast('Đã xóa') }} className="p-1.5 rounded-lg hover:bg-red-100 text-red-500"><Trash2 className="w-4 h-4" /></button>}
                          </td>
                        </tr>
                      )
                    })}
          ...
          {page === 'reports' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 print:hidden">
                <div><h2 className="text-2xl font-bold text-emerald-900">Báo cáo phụ huynh · In A4</h2><p className="text-sm text-emerald-600">Mỗi học sinh một trang khi in</p></div>
                <div className="flex gap-2">
                  <select value={week} onChange={e => setWeek(Number(e.target.value))} className="border border-emerald-200 rounded-lg px-3 py-2 text-sm bg-white">{Array.from({ length: classInfo.totalWeeks }, (_, i) => i + 1).map(w => <option key={w} value={w}>Tuần {w}</option>)}</select>
                  <button onClick={() => window.print()} className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-emerald-700"><Printer className="w-4 h-4" /> In A4</button>
                </div>
              </div>
          ...
          {page === 'monthly' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 print:hidden">
                <div><h2 className="text-2xl font-bold text-emerald-900">Báo cáo tháng</h2><p className="text-sm text-emerald-600">Tổng hợp 4 tuần gần nhất · họp PH</p></div>
                <button onClick={() => window.print()} className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-emerald-700"><Printer className="w-4 h-4" /> In báo cáo</button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[{ label: 'Sĩ số', value: students.filter(s => s.active).length }, { label: 'Tổng GD', value: transactions.length }, { label: 'Vi phạm', value: transactions.filter(t => t.points < 0).length }, { label: 'Khen', value: transactions.filter(t => t.points > 0).length }].map(c => (
                  <div key={c.label} className="bg-white rounded-2xl p-4 border border-emerald-100 shadow-sm"><p className="text-2xl font-bold text-emerald-900">{c.value}</p><p className="text-xs text-emerald-600">{c.label}</p></div>
                ))}
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-x-auto">
                <table className="w-full text-sm min-w-[700px]">
                  <thead className="bg-emerald-50"><tr><th className="px-3 py-2 text-left">STT</th><th className="px-3 py-2 text-left">Họ tên</th><th className="px-3 py-2 text-left">Tổ</th><th className="px-3 py-2 text-center">T{Math.max(1, week - 3)}</th><th className="px-3 py-2 text-center">T{Math.max(1, week - 2)}</th><th className="px-3 py-2 text-center">T{Math.max(1, week - 1)}</th><th className="px-3 py-2 text-center">T{week}</th><th className="px-3 py-2 text-center">Tổng</th></tr></thead>
                  <tbody>
                    {filteredStudents.map(s => {
          ...
          {page === 'settings' && canEditSettings(role) && (
            <div className="space-y-6 max-w-3xl">
              ...
              <div className={`border rounded-2xl p-5 flex items-start gap-3 ${isFirebaseConfigured ? 'bg-emerald-50 border-emerald-300' : 'bg-amber-50 border-amber-300'}`}>
                {isFirebaseConfigured ? <><span className="w-6 h-6 rounded-full bg-emerald-500 shrink-0 mt-0.5" /><div><p className="font-semibold text-emerald-800">Firebase đã cấu hình</p><p className="text-sm text-emerald-700 mt-1">quanlyhocsinh-48840 · realtime</p></div></> : <><WifiOff className="w-6 h-6 text-amber-600 shrink-0 mt-0.5" /><div><p className="font-semibold text-amber-800">Chưa kết nối Firebase</p></div></>}
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-6 space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  {([['schoolName', 'Tên trường *'], ['className', 'Tên lớp *'], ['homeroomTeacher', 'GVCN *'], ['schoolYear', 'Năm học'], ['week1StartDate', 'Ngày bắt đầu tuần 1'], ['slogan', 'Khẩu hiệu']] as const).map(([key, label]) => (
                    <div key={key}>
                ...
                <button onClick={async () => { if (isFirebaseConfigured) { try { await saveClassInfo(classInfo as unknown as Record<string, unknown>); showToast('Đã lưu lên Firebase') } catch { showToast('Lưu local – Firebase lỗi') } } else showToast('Đã lưu local') }} className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-emerald-700"><Save className="w-4 h-4" /> Lưu cài đặt</button>
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-6 flex flex-wrap gap-3">
                <button onClick={() => { downloadJSON(`backup-qlcn-${classInfo.className}.json`, { version: 2, exportedAt: new Date().toISOString(), classInfo, students, groups, transactions, lockedWeeks, lessons }); showToast('Đã tải backup JSON') }} className="border border-teal-600 text-teal-700 px-4 py-2 rounded-xl text-sm font-medium hover:bg-teal-50">Backup JSON</button>
                <button onClick={() => { const rows = [['ID','Học sinh','Tuần','Ngày','Sự kiện','Điểm','Người nhập']]; transactions.forEach(t => { const s = students.find(x => x.id === t.studentId); const r = rules.find(x => x.id === t.ruleId); rows.push([t.id, s?.fullName || '', String(t.weekNumber), t.date, r?.name || '', String(t.points), t.createdBy]) }); downloadCSV('giao-dich-diem.csv', rows); showToast('Đã xuất CSV điểm') }} className="border border-emerald-600 text-emerald-700 px-4 py-2 rounded-xl text-sm font-medium hover:bg-emerald-50">Xuất điểm CSV</button>
                {!demoLoaded && <button onClick={loadDemo} className="bg-teal-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-teal-700">Tải dữ liệu minh họa</button>}
                {demoLoaded && <button onClick={() => { if (confirm('Xóa dữ liệu minh họa?')) { setStudents([]); setTransactions([]); setDemoLoaded(false); showToast('Đã xóa') } }} className="bg-red-500 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-red-600">Xóa dữ liệu minh họa</button>}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 print:hidden">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] overflow-y-auto p-6">
            ...
            <div className="space-y-2">
              ...
              {rules.filter(r => r.active && canCreateScore(role, r.group)).length === 0 && <p className="text-sm text-gray-400 text-center py-4">Không có sự kiện trong quyền của bạn</p>}
            </div>
